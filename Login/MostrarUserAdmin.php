...
if (isset($_SESSION['id_admin']) && isset($_SESSION['nombreAdmin'])) {
    if (isset($_POST['eliminar']) && isset($_POST['id'])) {
        ...
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            header("Location: ../ViewsAdmin/AdminUsuarios.php?success=Cuenta eliminada exitosamente");
            exit();
        } else {
            // Redirigir con mensaje de error
            header("Location: ../ViewsAdmin/AdminUsuarios.php?error=Error al eliminar la cuenta");
            exit();
        }
    }
    ...
    if ($inc) {
        ...
        if ($resultado) {
            while ($row = $resultado->fetch_array()) {
                ...
                echo $id . " " . $usuario . " " . $nombre . " " . $correo . " " . 
                " <a href='../ViewsAdmin/AdminVerUser.php?id=$id'>Ver Perfil de Usuario</a> ";
